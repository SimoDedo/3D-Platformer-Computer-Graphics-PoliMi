...
enum VertexDescriptorElementUsage {POSITION, NORMAL, UV_0, UV_1, COLOR, TANGENT, OTHER};
...
struct VertexDescriptor {
	...
	VertexComponent UV_0;
	VertexComponent UV_1;
...
//		createInfo.ppEnabledExtensionNames = glfwExtensions;
	...
	void createImage(uint32_t width, uint32_t height,
		...
		allocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits,
											properties);
		VkResult resultImage = vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory);
		if (resultImage != VK_SUCCESS) {
			PrintVkError(resultImage);
			std::cout << "VkResult is " << resultImage << "\n";
	...
	// Control Wrapper
	void handleGamePad(int id,  glm::vec3 &m, glm::vec3 &r, bool &jump, bool &start) {
		...
		if(glfwJoystickIsGamepad(id)) {
			...
			if (glfwGetGamepadState(id, &state)) {
				...
				jump = jump | (bool)state.buttons[GLFW_GAMEPAD_BUTTON_A] | (bool)state.buttons[GLFW_GAMEPAD_BUTTON_B];
				start = start | (bool)state.buttons[GLFW_GAMEPAD_BUTTON_START];
			}
		}
	}
		
	void getSixAxis(float &deltaT, glm::vec3 &m, glm::vec3 &r, bool &jump, bool &start) {
		...
		jump = glfwGetKey(window, GLFW_KEY_SPACE) | glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS;
		start = glfwGetKey(window, GLFW_KEY_ENTER) == GLFW_PRESS;
		handleGamePad(GLFW_JOYSTICK_1,m,r,jump, start);
		handleGamePad(GLFW_JOYSTICK_2,m,r,jump, start);
		handleGamePad(GLFW_JOYSTICK_3,m,r,jump, start);
		handleGamePad(GLFW_JOYSTICK_4,m,r,jump, start);
	}
...
void VertexDescriptor::init(BaseProject *bp, std::vector<VertexBindingDescriptorElement> B, std::vector<VertexDescriptorElement> E) {
	...
	UV_0.hasIt = false; UV_0.offset = 0;
		...
		for(int i = 0; i < E.size(); i++) {
			switch(E[i].usage) {
			  ...
			  case VertexDescriptorElementUsage::NORMAL:
			    ...
			    break;
			  case VertexDescriptorElementUsage::UV_0:
			    if(E[i].format == VK_FORMAT_R32G32_SFLOAT) {
				  if(E[i].size == sizeof(glm::vec2)) {
					UV_0.hasIt = true;
					UV_0.offset = E[i].offset;
				  } else {
					std::cout << "Vertex UV_0 - wrong size\n";
				  }
				} else {
				  std::cout << "Vertex UV_0 - wrong format\n";
				}
			    break;
			  case VertexDescriptorElementUsage::UV_1:
				  if (E[i].format == VK_FORMAT_R32G32_SFLOAT) {
					  if (E[i].size == sizeof(glm::vec2)) {
						  UV_1.hasIt = true;
						  UV_1.offset = E[i].offset;
					  }
					  else {
						  std::cout << "Vertex UV_1 - wrong size\n";
					  }
				  }
				  else {
					  std::cout << "Vertex UV_1 - wrong format\n";
				  }
				  break;
...
//	std::cout << "Normal " << VD->Normal.hasIt << "," << VD->Normal.offset << "\n";	
	for (const auto& shape : shapes) {
		for (const auto& index : shape.mesh.indices) {
			...
			if(VD->UV_0.hasIt) {
				glm::vec2 *o = (glm::vec2 *)((char*)(&vertex) + VD->UV_0.offset);
				*o = texCoord;
			}
...
void Model<Vert>::loadModelGLTF(std::string file) {
	...
	for (const auto& mesh :  model.meshes) {
		...
		for (const auto& primitive :  mesh.primitives) {
			...
			const float* bufferTexCoords_0 = nullptr;
			const float *bufferTexCoords_1 = nullptr;
			
			bool meshHasPos = false;
			bool meshHasNorm = false;
			bool meshHasTan = false;
			bool meshHasUV_0 = false;
			bool meshHasUV_1 = false;
			...
			auto uIt_0 = primitive.attributes.find("TEXCOORD_0");
			if(uIt_0 != primitive.attributes.end()) {
				const tinygltf::Accessor &uvAccessor_0 = model.accessors[uIt_0->second];
				const tinygltf::BufferView &uvView_0 = model.bufferViews[uvAccessor_0.bufferView];
				bufferTexCoords_0 = reinterpret_cast<const float *>(&(model.buffers[uvView_0.buffer].data[uvAccessor_0.byteOffset + uvView_0.byteOffset]));
				meshHasUV_0 = true;
				cntUV_0 = uvAccessor_0.count;
				if(cntUV_0 > cntTot) cntTot = cntUV_0;
			} else {
				if(VD->UV_0.hasIt) {
					std::cout << "Warning: vertex layout has UV_0, but file hasn't\n";
				}
			}

			auto uIt_1 = primitive.attributes.find("TEXCOORD_1");
			if (uIt_1 != primitive.attributes.end()) {
				const tinygltf::Accessor& uvAccessor_1 = model.accessors[uIt_1->second];
				const tinygltf::BufferView& uvView_1 = model.bufferViews[uvAccessor_1.bufferView];
				bufferTexCoords_1 = reinterpret_cast<const float*>(&(model.buffers[uvView_1.buffer].data[uvAccessor_1.byteOffset + uvView_1.byteOffset]));
				meshHasUV_1 = true;
				cntUV_1 = uvAccessor_1.count;
				if (cntUV_1 > cntTot) cntTot = cntUV_1;
			}
			else {
				if (VD->UV_1.hasIt) {
					std::cout << "Warning: vertex layout has UV_1, but file hasn't\n";
				}
			}
			...
			for(int i = 0; i < cntTot; i++) {
				...
				if((i < cntUV_0) && meshHasUV_0 && VD->UV_0.hasIt) {
					glm::vec2 texCoord_0 = {
						bufferTexCoords_0[2 * i + 0],
						bufferTexCoords_0[2 * i + 1] 
					};
				...
				if ((i < cntUV_1) && meshHasUV_1 && VD->UV_1.hasIt) {
					glm::vec2 texCoord_1 = {
						bufferTexCoords_1[2 * i + 0],
						bufferTexCoords_1[2 * i + 1]
					};

					glm::vec2* o = (glm::vec2*)((char*)(&vertex) + VD->UV_1.offset);
					*o = texCoord_1;
				}
