...
layout(set = 1, binding = 2) uniform sampler2D tex_MRAO;
layout(set = 1, binding = 3) uniform sampler2D tex_e;
...
void main() {
	...
	vec3 N = fragNorm;  					// surface normal
	...
	//Emissive light
	vec3 Emissive = texture(tex_e, fragUV).rgb; 

	outColor = clamp(vec4(Direct + Ambient + Emissive + PlantPoint[0] + PlantPoint[1] + PlantPoint[2], texture(tex, fragUV).a), 0.0f, 1.0f);
